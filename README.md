...
- 支持中文/英文界面和深色/浅色主题
- 可选择自动、仅系统 DNS、内置 DoH 或内置 DoT；默认仅在本地 DNS 确认不可用时回退，短暂网络波动会保留系统解析
...
- 无 DNS 的在线环境如何运行？
  在“DNS 解析”选择器保留默认 `auto`：只有多个独立探测确认本地 DNS 不可用时，当前进程才会以固定地址完成真实 TLS DNS 查询，并选择延迟最低的内置 DoH 或 DoT；成功响应或 NXDOMAIN 会保留系统 DNS，超时、SERVFAIL 和短暂网络错误不会切换。若常规联网预检没有结果，auto 会先执行同样有时限的加密 DNS 校验。`system` 只使用系统 DNS，`doh` 强制使用内置 DoH，`dot` 强制使用内置 DoT。该机制不改写系统 resolver 文件；`-ut-dns` 仍是流媒体解锁的独立显式 DNS 覆盖。
- macOS 为什么会提示未签名？
