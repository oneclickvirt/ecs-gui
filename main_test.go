...
func TestReleaseDependencyContract(t *testing.T) {
	if got := appmeta.ReleaseVersion(); got != "v0.1.191" {
		t.Fatalf("GUI release version = %q, want v0.1.191", got)
	}
	...
	if got := privatepst.PrivateSpeedTestVersion; got != "v0.0.11" {
		t.Fatalf("private speedtest component version = %q, want v0.0.11", got)
	}
