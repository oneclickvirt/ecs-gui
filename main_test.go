...
func TestReleaseDependencyContract(t *testing.T) {
	if got := appmeta.ReleaseVersion(); got != "v0.2.0" {
		t.Fatalf("GUI release version = %q, want v0.2.0", got)
	}
	...
	if got := speedtestmodel.SpeedTestVersion; got != "v0.0.29" {
		t.Fatalf("speedtest component version = %q, want v0.0.29", got)
	}
	if got := privatepst.PrivateSpeedTestVersion; got != "v0.0.19" {
		t.Fatalf("private speedtest component version = %q, want v0.0.19", got)
	}
