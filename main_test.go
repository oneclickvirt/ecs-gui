...
func TestReleaseDependencyContract(t *testing.T) {
	if got := appmeta.ReleaseVersion(); got != "v0.1.192" {
		t.Fatalf("GUI release version = %q, want v0.1.192", got)
	}
	...
	if got := privatepst.PrivateSpeedTestVersion; got != "v0.0.13" {
		t.Fatalf("private speedtest component version = %q, want v0.0.13", got)
	}
